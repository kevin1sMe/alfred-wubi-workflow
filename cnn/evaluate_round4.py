...
def evaluate_round4():
    target_dir = Path("batch_100_test")
    ...
    # Initialize CNN Inference (Round 3 Model)
    try:
        # Load the model trained in Round 3 (on 100 images)
    ...
    results = []
    files = sorted(list(target_dir.glob("*.bmp")))
    ...
    for p in files:
        ...
        if cnn_ok: correct_counts["CNN (Round 3)"] += 1
    ...
    md_lines = []
    md_lines.append("# Round 4 Model Evaluation (Batch 100 Test)")
    md_lines.append(f"\n**Dataset**: {total} images from `{target_dir}`")
    md_lines.append("**Model**: CNN trained on Round 1+2+3 (100 images).")
    ...
    md_lines.append("\n## Detailed Comparison")
    md_lines.append("| File | Label | Template Match | EasyOCR | Tesseract | CNN (R3) |")
    md_lines.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
    for res in results:
    ...
    report_filename = "round4_test_report.md"
