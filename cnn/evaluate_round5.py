...
def evaluate_round5():
    ...
    try:
        # Load the model trained in Round 4 (on 200 images)
    ...
    md_lines = []
    md_lines.append("# Round 5 Model Evaluation (Batch 100 Test 2)")
    md_lines.append(f"\n**Dataset**: {total} images from `{target_dir}`")
    md_lines.append("**Model**: CNN trained on Round 1+2+3+4 (200 images).")
    ...
    md_lines.append("\n## Detailed Comparison")
    md_lines.append("| File | Label | Template Match | EasyOCR | Tesseract | CNN (R4) |")
    md_lines.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
    for res in results:
    ...
    report_filename = "round5_test_report.md"
